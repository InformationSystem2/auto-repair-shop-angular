...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, ButtonComponent, InputDirective],
  template: `
    <div class="w-full bg-white/70 dark:bg-slate-900/70 backdrop-blur-3xl rounded-[2rem] shadow-2xl shadow-slate-200/50 dark:shadow-[0_0_40px_-15px_rgba(0,0,0,0.5)] border border-white/50 dark:border-slate-800/80 overflow-hidden relative transition-all duration-300">
      ...
      <div class="px-8 sm:px-10 py-7 border-b border-slate-200/50 dark:border-slate-800/50">
        ...
        <p class="text-slate-500 dark:text-slate-400 mt-2 font-medium">Actualiza tus datos y opciones de seguridad de la cuenta.</p>
      </div>
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="px-8 sm:px-10 py-8 space-y-8">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-7">
          ...
          <div class="space-y-3 col-span-1 sm:col-span-2 md:col-span-1">
            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 ml-1">Nombres</label>
            <div class="relative group">
              <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-indigo-500 transition-colors">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
              </div>
              <input appInput type="text" formControlName="name" class="pl-12" placeholder="Tu nombre" />
            </div>
          </div>

          <!-- Last Name -->
          <div class="space-y-3">
            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 ml-1">Apellidos</label>
            <div class="relative group">
              <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-indigo-500 transition-colors">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
              </div>
              <input appInput type="text" formControlName="last_name" class="pl-12" placeholder="Tus apellidos" />
            </div>
          </div>

          <!-- Phone -->
          <div class="space-y-3">
            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 ml-1">Teléfono</label>
            <div class="relative group">
              <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-indigo-500 transition-colors">
          ...
          <!-- Password -->
          <div class="space-y-3">
            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 ml-1">Contraseña</label>
            <div class="relative group">
              <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-indigo-500 transition-colors">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
              </div>
              <input appInput type="password" formControlName="password" class="pl-12" placeholder="Dejar en blanco para conservar actual" />
            </div>
          </div>
